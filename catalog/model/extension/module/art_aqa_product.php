...
class ModelExtensionModuleArtAqaProduct extends Model {
	public function addAqaProduct($data) {					
		$this->db->query("INSERT INTO " . DB_PREFIX . "art_aqa_product SET email = '" . $this->db->escape($data['f_email']) . "', phone = '" . $this->db->escape($data['f_telephone']) . "', product_id = '" . (int)$data['product_id'] . "', question = '" . $this->db->escape($data['f_question']) . "', name = '" . $this->db->escape($data['f_name']) . "', communication = '" . $this->db->escape($data['f_communication']) . "', ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" . $this->db->escape($data['forwarded_ip']) . "', user_agent = '" . $this->db->escape($data['user_agent']) . "', accept_language = '" . $this->db->escape($data['accept_language']) . "', language_id = '" . (int)$this->config->get('config_language_id') . "', date_added = NOW(), date_fake = NOW()");
