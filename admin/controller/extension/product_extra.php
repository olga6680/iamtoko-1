...
class ControllerExtensionProductExtra extends ControllerCatalogProduct {
	...
	public function __construct($arg){
  	...
  	public function index() {
		...
		if(isset($this->request->get['type']) && $this->request->get['type'] == "change_seo"){
			echo $this->model_extension_product_extra->changeSeo((int)$this->request->get['product_id'], $this->request->get['seo'], isset($this->request->get['language'])?$this->request->get['language']:NULL);
			die();
		}
		...
		if(!isset($_SESSION['product_language'])){
	...
	public function ajaxify(){
		if(isset($this->request->get['product_id']) && (int)$this->request->get['product_id'] > 0){
			...
			$data['selected_language'] = isset($_SESSION['product_language'])?(int)$_SESSION['product_language']:(int)$this->config->get('config_language_id');
			$this->load->language('extension/product_extra');
			$this->load->model('extension/product_extra');
			/*controller-hook-ajaxify-1*/
			...
			$results = $this->model_extension_product_extra->getProducts($data = array('product_id'=>(int)$this->request->get['product_id']), (isset($_SESSION['product_language'])?$_SESSION['product_language']:$this->config->get('config_language_id')));
			...
			$data['text_default'] = $this->language->get('default_store_text');
			$this->load->model('setting/store');
			$data['stores'] = $this->model_setting_store->getStores();
			$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
			foreach ($results as $result) {
				/*controller-hook-ajaxify-loop*/
				$action = array();
				...
				$data['products'][] = array(
					...
					'subtract'     => ($result['status'] ? $this->language->get('text_yes') : $this->language->get('text_no')),
					...
					'keyword'    => $this->model_extension_product_extra->getAlias($result['product_id']),
			...
			if(isset($data['products']) && $data['products'] && count($data['products']) == 1){
	...
	private function specialPrices($product_id){
		...
		$output = $template->render(
			$this->registry->get('config')->get('template_directory') . 
			'extension/product_special_prices'
		);	
	...
	protected function validateSpecialForm(){
  	...
  	protected function getList() {			
		...
		if(isset($modules[0])){
			$module = $modules[0];
			$setting = json_decode($module['setting'], true);
			if (!empty($setting)&&isset($setting['product'])) { $data['featured'] = (array)$setting['product']; } else { $data['featured'] = array(); }
		}
		$data['selected_language'] = isset($_SESSION['product_language'])?(int)$_SESSION['product_language']:(int)$this->config->get('config_language_id');
		if(isset($this->request->get['rows'])){
			$_SESSION['current_rows'] = (int)$this->request->get['rows'];
		}

		if(!isset($_SESSION['current_rows'])){
			$_SESSION['current_rows'] = $this->config->get('config_limit_admin');
		}
		...
		if (isset($this->request->get['rows'])) {
			$url .= '&rows=' . urlencode($this->request->get['rows']);
			$_SESSION['current_rows'] = (int)$this->request->get['rows'];
		}
		...
		$filter_data = array(
			...
			'sort'            	=> $sort,
			'order'           	=> $order,
			'start'           	=> ($page - 1) * $_SESSION['current_rows'],//$this->config->get('config_admin_limit'),
			'limit'           	=> $_SESSION['current_rows']//$this->config->get('config_admin_limit')
		);
		...
		$results = $this->model_extension_product_extra->getProducts($filter_data, (isset($_SESSION['product_language'])?$_SESSION['product_language']:$this->config->get('config_language_id')));
		//Get categories
		$this->load->model('catalog/category');
		$list_categories = array();
		$categories = $this->model_catalog_category->getCategories(0);
		...
		$pagination->limit = $_SESSION['current_rows'];//$this->config->get('config_limit_admin');
		...
		$data['current_rows'] = $_SESSION['current_rows'];
