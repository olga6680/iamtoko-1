...
class ControllerExtensionModuleArtAqaProduct extends Controller {
	...
	public function getList() {
		...
		$data['sort_product_name'] = $this->url->link('extension/module/art_aqa_product', 'user_token=' . $this->session->data['user_token'] . '&sort=pd.name' . $url, true);
	...
	protected function getForm() {
		...
		if ($aqa_product_info) {
			...
			$data['product_name'] = $aqa_product_info['product_name']; 
			$data['product_href'] = HTTP_CATALOG . 'index.php?route=product/product&product_id=' . $aqa_product_info['product_id']; 
		} else {
			$data['question'] = '';
			$data['answer'] = '';
			$data['answer_name'] = '';
	...
	protected function sendMessage($aqa_product_id) {
		...
		if (!empty($aqa_product_info['email'])) {
			if ($this->config->get('module_art_aqa_product_mail_answer_status')) {
				...
				$message_temp['product'] = $aqa_product_info['product_name'];
				$message_temp['product_link'] = '<a href="' . $this->url->link('product/product', 'product_id=' . (int)$aqa_product_info['product_id']) . '">' . $aqa_product_info['product_name'] . '</a>';
