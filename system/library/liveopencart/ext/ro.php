...
class ro extends \liveopencart\lib\v0008\extension {
			...
			if ( $this->hasProductRO($product_id) ) {
				...
				foreach ($rovp_rows as $rovp_row) {
					...
					foreach ($query->rows as $row) {
						...
						$stock = '';
						$in_stock = false;
						if ( !empty($ro_settings['spec_ofs']) || (!empty($ro_settings['allow_zero_select']) && !empty($ro_settings['fade_out_of_stock_option_values'] ) ) ) {
							$in_stock = true;
							if ($row['quantity'] <= 0) {
								$in_stock = false;
							}
						...
						if ( !empty($ro_settings['spec_ofs']) ) {	
							if ($row['quantity'] <= 0) {
								...
								$stock = $this->language->get('ro_product')->get('text_instock');
							}
						}
	...
	public function getProductPriceWithRoByProductOptions($product_id, $options, $price, $special=false, $product_info=false) {
		if ($this->installed()) {
			...
			if ( $ro_settings && is_array($ro_settings) && count($options) ) {
				...
				$data = $this->calcProductPriceWithRO($price, $ro_combs, $special);
