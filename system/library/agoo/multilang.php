...
class agooMultilang extends Controller
{
	...
	public function __construct($registry) {
		...
		if (isset($_COOKIE['language']) && $_COOKIE['language'] != $this->config->get('config_language')) {
			$this->switchLanguage($this->langcode_all[$_COOKIE['language']]['language_id'], $_COOKIE['language']);
		}
        ...
        //if (!$ajax) {
            ...
            /*
            $switch_lang = true;
			if (isset($_COOKIE['lm_prefix']) && urldecode($_COOKIE['lm_prefix']) != $multi_switch['prefix'] && isset($_SERVER['HTTP_REFERER'])) {

                    $referal = str_ireplace(array('http://', 'https://'), array('', ''), $_SERVER['HTTP_REFERER']);
                    $referal_uri = trim(str_ireplace($_COOKIE['lm_prefix'], '', $referal), '/');
